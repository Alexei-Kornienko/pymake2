...
def compile(compiler, flags, sources, objects):
    ...
    for i, item in enumerate(srcs):
        ...
        if srcFile != objFile:
            ...
            print(
                'source file %s and object file %s do not match. Make sure that the source and the object files lists are correspondent' % (
                item, objs[i]))
            return False
...
def link(linker, flags, objects, executable):
    ...
    if linkFlag:
        ...
        cmd = '{linker} {flags} {objs} -o {executable}'.format(linker=linker, flags=flags, objs=objs,
